...
(function () {
    ...
    const CHART_LAYOUT = {
        line: {padLeft: 50, padRight: 10, padTop: 20, padBottom: 28},
        bar: {padLeft: 50, padRight: 10, padTop: 10, padBottom: 28}
    };
    ...
    function pickFirstExisting(list) {
        for (const tf of list) {
            ...
            const d = timeframes[norm];
            if (d && ((d.prices && d.prices.length) || (d.volumes && d.volumes.length))) return norm;
        }
        return '1d';
    }
    ...
    function fmt(n, dpAuto = true) {
        if (n == null) return '-';
        if (typeof n === 'number') return n.toLocaleString(undefined, {maximumFractionDigits: (dpAuto ? (Number.isInteger(n) ? 0 : 2) : 2)});
        return String(n);
    }
    ...
    function hasChartData() {
        return TF_ORDER.some(tf => {
            const d = timeframes[tf];
            return d && d.prices && d.prices.length;
        });
    }
    ...
    function hasVolumeData() {
        return TF_ORDER.some(tf => {
            const d = timeframes[tf];
            return d && d.volumes && d.volumes.length;
        });
    }

    function buildCard(title, primary, pairs) {
        if (primary == null || primary === '-') primary = '-';
        return `<div class="card compact"><div class="label">${title}</div><div class="metric">${primary}</div>` + (pairs && pairs.length ? `<div class='separator'></div>${pairs.map(p => `<div class='inline-pair'><span>${p[0]}</span><span>${p[1]}</span></div>`).join('')}` : '') + `</div>`;
    }
    ...
    function drawAxes(ctx, w, h, layout, pal) {
        ctx.strokeStyle = pal.grid;
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(layout.padLeft, layout.padTop - 5);
        ctx.lineTo(layout.padLeft, h - layout.padBottom + 5);
        ctx.moveTo(layout.padLeft - 5, h - layout.padBottom);
        ctx.lineTo(w - layout.padRight + 2, h - layout.padBottom);
        ctx.stroke();
    }

    function drawLineChart(canvas, data) {
        const {ctx, w, h} = prepareCanvas(canvas);
        const {padLeft, padRight, padTop, padBottom} = CHART_LAYOUT.line;
        ctx.clearRect(0, 0, w, h);
        ctx.font = '11px system-ui';
        ctx.fillStyle = PALETTE.text;
        if (!data || !data.prices || !data.prices.length) {
            ctx.fillText('No data', padLeft, padTop + 10);
            return;
        }
        const prices = data.prices, ts = data.timestamps || prices.map((_, i) => i), n = prices.length,
            single = n === 1;
        const minP = Math.min(...prices), maxP = Math.max(...prices), range = (maxP - minP) || 1;
        for (let i = 0; i <= 5; i++) {
            const y = padTop + (h - padTop - padBottom) * (i / 5);
            ctx.strokeStyle = PALETTE.grid;
            ctx.beginPath();
            ctx.moveTo(padLeft, y);
            ctx.lineTo(w - padRight, y);
            ctx.stroke();
            const val = maxP - range * (i / 5);
            ctx.fillStyle = PALETTE.text;
            ctx.textAlign = 'left';
            ctx.fillText(fmt(val, false), 4, y + 4);
        }
        ctx.textAlign = 'center';
        if (single) {
            const xC = padLeft + (w - padLeft - padRight) / 2;
            ctx.fillText(labelFor(ts[0], currentPriceTf), xC, h - 8);
        } else {
            [0, Math.floor(n / 2), n - 1].forEach(idx => {
                const frac = idx / (n - 1);
                const x = padLeft + (w - padLeft - padRight) * frac;
                ctx.fillText(labelFor(ts[idx], currentPriceTf), x, h - 8);
            });
        }
        ...
        if (!single) {
            ...
            prices.forEach((p, i) => {
                const frac = i / (n - 1);
                const x = padLeft + (w - padLeft - padRight) * frac;
                const y = padTop + (h - padTop - padBottom) * (1 - (p - minP) / range);
                if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
            });
            ctx.stroke();
        }
        const hi = (highlightPriceIndex != null && highlightPriceIndex < n) ? highlightPriceIndex : n - 1;
        const frac = single ? 0.5 : (n > 1 ? hi / (n - 1) : 0.5);
        const px = padLeft + (w - padLeft - padRight) * frac;
        const py = padTop + (h - padTop - padBottom) * (1 - (prices[hi] - minP) / range);
        ctx.fillStyle = PALETTE.line;
        ctx.beginPath();
        ctx.arc(px, py, single ? 6 : 5, 0, Math.PI * 2);
        ctx.fill();
        ...
        if (single) {
            ctx.textAlign = 'left';
            ctx.fillStyle = PALETTE.text;
            ctx.fillText(`Value: ${fmt(prices[0])}`, padLeft + 6, padTop + 14);
        }
    }

    function drawBarChart(canvas, data) {
        const {ctx, w, h} = prepareCanvas(canvas);
        const {padLeft, padRight, padTop, padBottom} = CHART_LAYOUT.bar;
        ctx.clearRect(0, 0, w, h);
        ctx.font = '11px system-ui';
        ctx.fillStyle = PALETTE.text;
        ...
        const vols = data.volumes, ts = data.timestamps || vols.map((_, i) => i), n = vols.length, single = n === 1;
        ...
        if (single) {
            const xc = padLeft + (w - padLeft - padRight) / 2;
            ctx.fillText(labelFor(ts[0], currentVolumeTf), xc, h - 8);
        } else {
            [0, Math.floor(n / 2), n - 1].forEach(idx => {
                const frac = idx / (n - 1);
                const x = padLeft + (w - padLeft - padRight) * frac;
                ctx.fillText(labelFor(ts[idx], currentVolumeTf), x, h - 8);
            });
        }
        drawAxes(ctx, w, h, CHART_LAYOUT.bar, PALETTE);
        const availW = w - padLeft - padRight;
        const barW = single ? Math.min(60, availW * 0.25) : Math.max(2, (availW / n) * 0.6);
        vols.forEach((v, i) => {
            const frac = single ? 0.5 : i / (n - 1 || 1);
            const xCenter = padLeft + (w - padLeft - padRight) * frac;
            const hScale = v / maxV;
            const barH = (h - padTop - padBottom) * hScale;
            const y = padTop + (h - padTop - padBottom - barH);
            const x = xCenter - barW / 2;
            ctx.fillStyle = (highlightVolumeIndex === i) ? PALETTE.line : PALETTE.bar;
            ctx.fillRect(x, y, barW, barH);
            if (single) {
                ctx.textAlign = 'left';
                ctx.fillStyle = PALETTE.text;
                ctx.fillText(`Vol: ${fmt(v)}`, padLeft + 6, padTop + 14);
            }
        });
        if (highlightVolumeIndex != null && highlightVolumeIndex < n) {
            const i = highlightVolumeIndex;
            const v = vols[i];
            const frac = single ? 0.5 : i / (n - 1 || 1);
            const xc = padLeft + (w - padLeft - padRight) * frac;
            const hScale = v / maxV;
            const barH = (h - padTop - padBottom) * hScale;
            const y = padTop + (h - padTop - padBottom - barH);
            const x = xc - barW / 2;
    ...
    function renderCharts() {
        ...
        const p = timeframes[currentPriceTf];
        const v = timeframes[currentVolumeTf];
    ...
    function enableInteraction() {
        if (!hasChartData() && !hasVolumeData()) return;

        function handler(kind) {
            const isPrice = kind === 'price';
            const canvas = isPrice ? priceChart : volumeChart;
            return function (ev) {
                const tf = isPrice ? currentPriceTf : currentVolumeTf;
                const data = timeframes[tf];
                if (!data) return;
                const series = isPrice ? (data.prices || []) : (data.volumes || []);
                const n = series.length;
                if (!n) return;
                const rect = canvas.getBoundingClientRect();
                const layout = isPrice ? CHART_LAYOUT.line : CHART_LAYOUT.bar;
                const usable = rect.width - layout.padLeft - layout.padRight;
                let idx = 0;
                if (n > 1 && usable > 1) {
                    const x = ev.clientX - rect.left;
                    const clamped = Math.min(Math.max(x - layout.padLeft, 0), usable);
                    idx = Math.min(n - 1, Math.max(0, Math.round((clamped / usable) * (n - 1))));
                }
                if (isPrice) highlightPriceIndex = idx; else highlightVolumeIndex = idx;
                renderCharts();
                const tip = isPrice ? priceTooltip : volumeTooltip;
                const ts = data.timestamps && data.timestamps[idx];
                tip.innerHTML = isPrice ? `<strong>${fmt(series[idx])}</strong><br><span style='opacity:.75'>${ts ? fmtTime(new Date(ts).toISOString()) : ''}</span>` : `<strong>Vol ${fmt(series[idx])}</strong><br><span style='opacity:.75'>${ts ? fmtTime(new Date(ts).toISOString()) : ''}</span>`;
                tip.hidden = false;
                const wrap = isPrice ? priceWrap : volumeWrap;
                const wrect = wrap.getBoundingClientRect();
                let tx = ev.clientX - wrect.left;
                let ty = ev.clientY - wrect.top - 8;
                if (tx < 40) tx = 40; else if (tx > wrect.width - 40) tx = wrect.width - 40;
                if (ty < 32) ty = 32;
        ...
        function attach(c, kind) {
            const mv = handler(kind);
            c.addEventListener('pointerdown', e => {
                c.setPointerCapture(e.pointerId);
                mv(e);
            });
            c.addEventListener('pointermove', e => {
                if (c.hasPointerCapture(e.pointerId)) mv(e);
            });
            ['pointerup', 'pointerleave', 'pointercancel'].forEach(ev => c.addEventListener(ev, () => {
                if (kind === 'price') {
                    highlightPriceIndex = null;
                    priceTooltip.hidden = true;
                } else {
                    highlightVolumeIndex = null;
        ...
        attach(priceChart, 'price');
        attach(volumeChart, 'volume');
    }

    function initTimeframeButtons() {
        document.querySelectorAll('.timeframe-switch').forEach(group => {
            const chart = group.dataset.chart;
            group.querySelectorAll('button[data-tf]').forEach(btn => {
                const tf = normalizeTf(btn.dataset.tf);
                const has = !!timeframes[tf] && ((timeframes[tf].prices && timeframes[tf].prices.length) || (timeframes[tf].volumes && timeframes[tf].volumes.length));
                if (!has) {
    ...
    function prepareAndRender(retry = 0) {
        ...
        prepareCanvas(priceChart);
        prepareCanvas(volumeChart);
        renderCharts();
        enableInteraction();
    }
